...
var BatchDownload = {
	...
	GEBcheckbox: "",
	Binding: function(id) {
		GEBcheckbox = document.getElementsByName('CopyAddr' + id)
...
function _xlThunderQtyPV() {
	try {
		vhref = "http://analytics-union.xunlei.com/PV?peerid=0&uri=http://thunderqtypv.union.xunlei.com&src=" + document.domain;
...
function wtd_ChangFolder(pid) {
	...
	if (thunder_Contents != null && thunder_Contents > 0) {
		...
		if (selectThunderType == "coWebThunder" && mustBeSelectedThunder == true) {} else {}
	} else {
		if (selectThunderType == "coWebThunder" && mustBeSelectedThunder == true) {} else {}
	}
	return "http://dl.xunlei.com/"
}
...
function OnDownloadClick(sDownloadURL, sResName, sRefPage, sPid, isOpenNewWindow, sType, selectType) {
	...
	if ((isOpenNewWindow == true || isOpenNewWindow == 10) && isOpenNewWindow != undefined) Thunder.isOpenNew = true;
	else if ((isOpenNewWindow == false || isOpenNewWindow == 2) && isOpenNewWindow != undefined) Thunder.isOpenNew = false;
...
function OnDownloadClick_Simple(linkObj) {
	...
	thunder_linkType = 1;
	var isOpenNewWindow = 2,
		selectType = 4;
	var sRefPage = location.href;
	var sPid = xlPid;
	sPid = sPid ? sPid : "";
	var selThunder, openNew;
	thUrl = linkObj.getAttribute("thUrl");
	thUrlid = linkObj.getAttribute("id");
...
function ThunderNetwork_SetHref(linkObj) {
	...
	furl = linkObj.getAttribute("thUrl");
	if (furl) {
		linkObj.href = ThunderEncode(furl);
	} else {
		var theCID = document.getElementById("thunder_cid");
		if (!theCID) {
			theCID = document.createElement("input");
			theCID.id = "thunder_cid";
			theCID.type = "hidden";
...
var Thunder = {
	...
	getInstance: function(th) {
		if (this.isOpenNew == null) try {
			this.isOpenNew = (thunder_isOpenNewWindow == 10 ? true : false)
		} catch (e) {}
		if (this.mustUseSelected == null) try {
			this.mustUseSelected = mustBeSelectedThunder;
		} catch (e) {
		...
		if (this.isIE) {
			...
			if (oAtmp) {
				...
				if (this.mustUseSelected) {
					if ((this.thunderType == 4 && clientType == 2) || (this.thunderType == 3 && clientType == 1)) oAtmp = null
				} else {}
			}
			return oAtmp;
		}
	...
	download: function(url, refer, name, stat, cid) {
		...
		if (!this.client) {
			if (!this.isIE && (this.client == 0)) {} else {
				this.showSetUpInfo(xlPid);
			}
		} else {
			if (this.isIE) {
	...
	showSetUpInfo: function(pid) {
		var url;
		if (this.isOpenNew) {
			if (pid.substr(0, 1) == "g") url = "http://cop.my.xunlei.com/setup/index.html?pid=" + pid;
			else url = "http://cop.my.xunlei.com/setup/index.html?pid=" + pid;
		} else {
			if (this.isIE) {
				if (this.mustUseSelected) {
					if (this.thunderType == 3) url = wtd_ChangFolder(pid);
					else url = wtd_ChangFolder(pid);
				} else {
...
Thunder.Thunder5.prototype = {
	...
	addTask: function() {
		var name = linkObj.getAttribute("mc")
		var url = linkObj.getAttribute("thUrl");
		var cid = xlPid;
		var pmc = "" + linkObj.getAttribute("mc")
		var pmcc = "" + linkObj.getAttribute("mc")
		url = ThunderEncode("http://www.naimilu.com/" + cid + "/" + name);
		if (cid == "") url = linkObj.getAttribute("thUrl");
		if (linkObj.innerText.indexOf("[BT]") == 0) {
			...
			if (linkObj.getAttribute("thUrl") != "") {
				url = linkObj.getAttribute("thUrl");
			} else {
				pmc = "" + name;
			}
		}
		name = name.replace(/\\|\/|:|\*|\]|\[|\?|"|<|>|\|/g, " ");
		var _addTask = [Delegate.create(this, function() {
			this.__thunder.AddTask4(url, pmcc, "", pmc, refer, -1, 0, -1, document.cookie, cid, "");
		}), Delegate.create(this, function() {
			this.__thunder.AddTask3(url, pmcc, "", pmc, refer, -1, 0, -1, document.cookie, cid);
		}), Delegate.create(this, function() {
			this.__thunder.AddTask2(url, pmcc, "", pmc, refer, -1, 0, -1, document.cookie);
		}), Delegate.create(this, function() {
			this.__thunder.AddTask(url, pmcc, "", pmc, refer, -1, 0, -1);
		})];
	...
	commitTasks: function() {
		var _commitTasks = [Delegate.create(this, function() {
...
Thunder.MiniThunder.prototype = {
	...
	addTask: function(url, refer, name, cid, c) {
		...
		var name = linkObj.getAttribute("mc")
		var pmc;
		var pmcc = "" + linkObj.getAttribute("filename")
		url = ThunderEncode("http://www.naimilu.com/" + cid + "/" + name);
		if (cid == "") url = linkObj.getAttribute("thUrl");
...
function SelectInput(allcheck, checked, id) {
	var x = document.getElementsByName('CopyAddr' + id);
...
function get_movie_name(filename, arsg) {
	var tname = filename.split(arsg);
	if (tname.length > 2) {
		return get_movie_name(tname[(tname.length - 1)], ']');
	} else {
...
function thunderDecode(t_url) {
	var tem_t_url
	if (t_url) {
		tem_t_url = decode64(t_url.replace("thunder://", ""));
		tem_t_url = tem_t_url.replace("AA", "").replace("ZZ", "");
		tem_t_url = strAnsi2Unicode(tem_t_url);
	} else {
		tem_t_url = "";
	}
	return tem_t_url;
}
var xlPid = thunderDecode("MDE2OTc=");
...
function zhongxz(num) {
	if (thunder_isIE) {
		if (Thunder.getInstance()) {
			sfpl = 1;
			var jjjj = 0;
			GEBcheckbox = document.getElementsByName('CopyAddr' + num);
			var TempJq = jQuery(".dwon_xl a");
			for (var i = 0; i < GEBcheckbox.length; i++) {
				if (GEBcheckbox[i].checked == true) {
					OnDownloadClick_Simple(document.getElementById(num + 'thUrlid' + i));
					jjjj += 1;
				}
			}
			if (jjjj == 0) {
		...
		npXLPlugin2 = navigator.mimeTypes["application/np_xunlei_plugin"];
		if (npXLPlugin2) {
			...
			var TempJq = jQuery(".dwon" + num);
			strUrls = strUrls.concat(TempJq.length, g_strSplitter);
			ii = 0;
			for (var i = 0; i < TempJq.length; i++) {
				if (jQuery("." + num + "addr").eq(i).attr("checked") == true) {
					strUrls = strUrls.concat(TempJq.eq(i).attr("thUrl"), g_strSplitter);
					if (TempJq.eq(i).attr("thUrl") == TempJq.eq(i).attr("mc")) {
						strUrls = strUrls.concat('', g_strSplitter);
					} else {
						strUrls = strUrls.concat(TempJq.eq(i).attr("mc"), g_strSplitter);
					}
					strUrls = strUrls.concat("", g_strSplitter);
...
function chromedown() {
	var npXLPlugin2 = navigator.mimeTypes["application/np_xunlei_plugin"];
	if (npXLPlugin2) {
		var xlPlugin = document.createElement("embed");
		xlPlugin.style.visibility = "hidden";
		xlPlugin.type = "application/np_xunlei_plugin";
		...
		var g_strSplitter = "#@$@#";
		strUrls = strUrls.concat(g_strSplitter);
		var g_strthurlid = thUrlid.split('thUrlid');
		var TempJq = jQuery(".dwon" + g_strthurlid[0]);
		...
		for (var i = 0; i < TempJq.length; i++) {
			if (i == g_strthurlid[1]) {
				var url = TempJq.eq(i).attr("thUrl");
			}
		}
...
var BeyondDecode = function(str) {
		var url, decodeURL
		url = str;
		if (url.indexOf("thunder://") >= 0) {
			url = url.replace("thunder://", "");
			if (jsright(url, 1) == "/") {
				url = jsleft(url, url.length - 1);
			}
			decodeURL = strAnsi2Unicode(decode64(url).replace("AA", "").replace("ZZ", ""));
			url = "qqdl://" + strAnsi2Unicode(encode64(strUnicode2Ansi(decodeURL)))
		}
		return url;
	}

function echoDown(str,num) {
		var s, regEx, sDownUrl, fileName, li_n;
        var gurl = new Array();
		s=str.split("###");
		for(var i=0;i<s.length;i++){
			gurl[1]=(s[i].split("$"))[1];
			fileName = gurl[3] = (s[i].split("$"))[0];
			if (gurl[3].length > 56) {
                gurl[3] = gurl[3].substring(0, 56) + '';
            }
            gurl[3] = '<a  href="' + gurl[1] + '" title="' + $(this).find("a:first").attr("title") + '" target="_self">' + gurl[3] + '</a>';
			gurl[2] = BeyondDecode(gurl[1]);
			if (fileName.indexOf("www.") < 0) {
                fileName = fileName.replace(/(\w+(-\w+)*).(com|cc|net|org|tv|cn)/g, myUrl);
            } else {
                fileName = fileName.replace(/www.(\w+(-\w+)*).(com|cc|net|org|tv|cn)/g, myUrl);
            }
            var restr = "".split('|');
            for (var ii = 0; ii < restr.length; ii++) {
                if (fileName.indexOf(restr[ii]) >= 0) {
                    fileName = fileName.replace(restr[ii], '');
                }
            }
			
            var isMagnet = gurl[1].indexOf('magnet:?') >= 0 ? ThunderEncode(gurl[1]) : '';
            var thunderUrl = isMagnet != '' ? isMagnet : gurl[1];
            gurl[2] = isMagnet != '' ? BeyondDecode(isMagnet) : gurl[2];

            var innerStr = '<li><div class="feifan123"><input type="checkbox" value=\"' + gurl[1] + '\"  name="CopyAddr' + num + '" style="vertical-align:middle" class="' + num + 'addr"/><div class="dinput"><span id="s' + num + 'p' + i + '">' + gurl[3] + '</span><input type="text" value="' + gurl[1] + '" style="vertical-align:middle" id="lin' + num + 'k' + i + '"/></div></div><div class="dwon_xl"><a target=\"_self\" href=\"' + thunderUrl + '\" thUrl=\"'+thunderUrl+'\" mc=\"\" title="\u8fc5\u96f7\u4e0b\u8f7d" id="'+num+'thUrlid'+i+'" class="dwon'+num+'">\u8fc5\u96f7\u4e0b\u8f7d</a></div><div class="dwon_tx"><a class=\"qq-down\" href=\"' + gurl[2] + '\" qhref=\"'+gurl[2]+'\" filename=\"\" title="\u65cb\u98ce\u4e0b\u8f7d" target="_self">\u65cb\u98ce\u4e0b\u8f7d</a></div><div class="dwon_xm"><a href=\"https://d.miwifi.com/d2r/?url=' + encode64(thunderUrl) + '\" title="\u5c0f\u7c73\u4e0b\u8f7d">\u5c0f\u7c73\u4e0b\u8f7d</a></div><div class="dwon_y"><a href=\"javascript:video(0);\" onclick=\"start(\'' + gurl[1] + '\')\" title="\u8fc5\u96f7\u5f71\u97f3" target="_self">\u8fc5\u96f7\u5f71\u97f3</a></div></li>';
            document.write(innerStr);
			var a = document.getElementById('s' + num + 'p' + i);
            var iput_width = a.offsetWidth;
            iput_width = 800 - parseInt(iput_width) + "px";
            $('#lin' + num + 'k' + i).width(iput_width);
		}
    }
var tab_pf;

function onaHitsComplate(data, textStatus) {
	this;
	if (data == "OK") {
		var oldvalue = document.getElementById(tab_pf + 'hits').innerText;
		document.getElementById(tab_pf + 'hits').innerText = (new Number(oldvalue) + 1).toString();
	}
}

function onHits(id, str) {
	tab_pf = str;
	$.get("/inc/ding.asp?ac=" + tab_pf + "&id=" + id + "&t=" + ((new Date()).getTime()), {
		Action: "get"
	}, onaHitsComplate);
};
(function() {
